...
void MemReporterBase::print_malloc(const MemoryCounter* c, MemTag mem_tag) const {
  ...
  if (mem_tag != mtNone) {
    out->print("(%s%zu%s tag=%s", alloc_type,
      amount_in_current_scale(amount), scale, name_for(mem_tag));
  } else {
...
void MemSummaryReporter::report() {
  ...
  int current_number_of_tags = MemTagFactory::number_of_tags();
  for (int index = 0; index < current_number_of_tags; index ++) {
...
void MemSummaryReporter::report_summary_of_tag(MemTag mem_tag,
  ...
  out->print("-%*s (", indent - 2, name_for(mem_tag));
...
int MemDetailReporter::report_virtual_memory_allocation_sites()  {
  ...
  while ((virtual_memory_site = virtual_memory_itr.next()) != nullptr) {
    ...
    INDENT_BY(29,
      ...
      if (mem_tag != mtNone) {
        out->print(" Tag=%s", name_for(mem_tag));
      }
      out->print_cr(")");
    )
    out->cr();
...
void MemDetailReporter::report_virtual_memory_region(const VirtualMemoryRegion* rgn) {
  ...
  out->print(" for %s", name_for(rgn->mem_tag()));
  if (stack->is_empty()) {
    out->cr();
  } else {
    out->print_cr(" from");
...
void MemSummaryDiffReporter::print_malloc_diff(size_t current_amount, size_t current_count,
  ...
  if (mem_tag != mtNone && mem_tag != mtThread) {
    out->print(" type=%s", name_for(mem_tag));
  }
